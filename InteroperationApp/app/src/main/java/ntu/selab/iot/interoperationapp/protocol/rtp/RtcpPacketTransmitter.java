...
public class RtcpPacketTransmitter extends Thread {
    ...
    private final static String TAG = "RtcpPacketTransmitter";
    /**
         * Remote address
         */
     ...
     * @throws IOException
     */
    public RtcpPacketTransmitter(InetAddress  address, int port, RtcpSession rtcpSession)
            throws IOException {
        super();
     ...
     */
         ...
         */
        public void run() {
            Log.d(TAG,"Qos-RTCP Transmitter start");
                try {
                    Log.d(TAG,"Qos-RTCP Transmitter Send a SDES packet before");
            // Send a SDES packet
            sendSdesPacket();
                    Log.d(TAG,"Qos-RTCP Transmitter Send a SDES packet after");
            boolean terminate = false;
            while (!terminate) {
                Log.d(TAG,"Qos-RTCP Transmitter in while");
                try {
...
//                        }
     ...
     */
        private void transmit(RtcpCompoundPacket packet) {
            Log.d(TAG,"Qos-RTCP Transmitter: Prepare data to be sent");
            ...
            Log.d(TAG,"Qos-RTCP Transmitter: Update statistics");
                // Update statistics
            stats.numBytes += packet.length;
            stats.numPackets++;
            rtcpSession.updateavgrtcpsize(packet.length);
            rtcpSession.timeOfLastRTCPSent = rtcpSession.currentTime();
            Log.d(TAG,"Qos-RTCP Transmitter:  Send data over UDP");
                // Send data over UDP
                try {
                        send(remoteAddress, remotePort, data);
                } catch(IOException e) {
//                        if (logger.isActivated()) {
//                                logger.error("Can't send the RTCP packet", e);
//                        }
    ...
    private void transmit(byte packet[]) {
        ...
        // Send data over UDP
        try {
        	send(remoteAddress, remotePort, packet);
        } catch (IOException e) {
//            if (logger.isActivated()) {
//                logger.error("Can't send the RTCP packet", e);
//            }
    ...
    private void send(InetAddress remoteAddr, int remotePort, byte[] data) throws IOException{
        Log.d(TAG,"Qos-RTCP Transmitter:  Send");
    	 if (data == null) {
             Log.d(TAG,"Qos-RTCP Transmitter: data == null");
             return;
        }
        Log.d(TAG,"Qos-RTCP Transmitter: data != null");
     if (datagramChannel  != null) {
         ...
         buf.put(data);
         buf.flip();
         Log.d(TAG,"Qos-RTCP Transmitter: datagramChannel != null");
//             DatagramPacket packet = new DatagramPacket(data, data.length, remoteAddr, remotePort);
         Log.d(TAG,"Qos-RTCP Transmitter: data will be sent");
         datagramChannel.send(buf, new InetSocketAddress(remoteAddr, remotePort));
//             datagramChannel.socket().send(packet);
            Log.d(TAG, "Qos Send RTCP packet");
     } else {
         Log.d(TAG,"Qos-RTCP Transmitter: datagramChannel == null");
    ...
    private void sendSdesPacket() {
        Log.d(TAG,"Qos-RTCP Transmitter: Start send a SDES packet");
        // Create a report
        Vector<RtcpSdesPacket> repvec = makereports();
        RtcpPacket packets[] = new RtcpPacket[repvec.size()];
        repvec.copyInto(packets);
        Log.d(TAG,"Qos-RTCP Transmitter: Create a RTCP compound packet");
        // Create a RTCP compound packet
        RtcpCompoundPacket cp = new RtcpCompoundPacket(packets);
        Log.d(TAG,"Qos-RTCP Transmitter: Assemble the RTCP packet");
        // Assemble the RTCP packet
        int i = cp.calcLength();
        cp.assemble(i, false);
        Log.d(TAG,"Qos-RTCP Transmitter:  Send the RTCP packe");
        // Send the RTCP packet
        transmit(cp);
    }
}